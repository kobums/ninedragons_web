...
interface KGState {
  ...
  gameOver: KGGameOverPayload | null;
  error: string | null;
...
// 트릭·비드 진행은 보드가 스냅샷(trick·players[].bid)으로 직접 그린다.
const TOAST_SKIP: ReadonlySet<string> = new Set(['react']);

// 서버가 상태 변경마다 개인화 전체 스냅샷(kg_game_state)을 보내므로
...
export const useSkullKingGameState = (lastMessage: KGMessage | null) => {
  const [state, setState] = useState<KGState>(initialState);
  const [toasts, setToasts] = useState<KGToast[]>([]);
  ...
  useEffect(() => {
    ...
    switch (lastMessage.type) {
      case 'kg_player_joined': {
        const payload = lastMessage.payload as { sessionId?: string };
        if (payload?.sessionId) {
          saveSessionId(KG_SESSION_KEY, payload.sessionId);
        }
      ...
      case 'kg_game_state': {
        const game = lastMessage.payload as KGGameState;
        setState((prev) => ({
          ...prev,
          game,
          hasJoined: true,
      ...
      case 'kg_event': {
        const event = lastMessage.payload as KGEvent;
      ...
      case 'kg_game_over': {
        const gameOver = (lastMessage.payload ?? {}) as KGGameOverPayload;
        clearSessionId(KG_SESSION_KEY);
        setState((prev) => ({ ...prev, gameOver, someoneDisconnected: false }));
        break;
      }

      case 'kg_player_disconnected':
        setState((prev) => ({ ...prev, someoneDisconnected: true }));
        break;

      case 'kg_player_reconnected':
        // 정확한 접속 상태는 뒤따르는 kg_game_state 가 채운다
        setState((prev) => ({ ...prev, someoneDisconnected: false }));
        break;

      case 'kg_session_expired':
        clearSessionId(KG_SESSION_KEY);
        setState((prev) => {
