...
interface SUState {
  ...
  gameOver: SUGameOverPayload | null;
  error: string | null;
  // 게임 중 사람 좌석이 끊겨 재접속 대기 중인지 (배너용)
  someoneDisconnected: boolean;
}

const initialState: SUState = {
  hasJoined: false,
  game: null,
  gameOver: null,
  error: null,
...
// 가져오기·내려놓기는 보드가 스냅샷(lastAction·market·faceUp)으로 직접 그리지만,
// 안티 적립·대주주 교체처럼 놓치면 안 되는 사건은 토스트로도 남긴다.
const TOAST_SKIP: ReadonlySet<string> = new Set(['react']);

// 서버가 상태 변경마다 개인화 전체 스냅샷(su_game_state)을 보내므로
...
export const useStartupsGameState = (lastMessage: SUMessage | null) => {
  const [state, setState] = useState<SUState>(initialState);
  const [toasts, setToasts] = useState<SUToast[]>([]);
  ...
  useEffect(() => {
    ...
    switch (lastMessage.type) {
      case 'su_player_joined': {
        const payload = lastMessage.payload as { sessionId?: string };
        if (payload?.sessionId) {
          saveSessionId(SU_SESSION_KEY, payload.sessionId);
        }
      ...
      case 'su_game_state': {
        const game = lastMessage.payload as SUGameState;
        setState((prev) => ({
          ...prev,
          game,
          hasJoined: true,
      ...
      case 'su_event': {
        const event = lastMessage.payload as SUEvent;
      ...
      case 'su_game_over': {
        const gameOver = (lastMessage.payload ?? {}) as SUGameOverPayload;
        clearSessionId(SU_SESSION_KEY);
        setState((prev) => ({ ...prev, gameOver, someoneDisconnected: false }));
        break;
      }

      case 'su_player_disconnected':
        setState((prev) => ({ ...prev, someoneDisconnected: true }));
        break;

      case 'su_player_reconnected':
        // 정확한 접속 상태는 뒤따르는 su_game_state 가 채운다
        setState((prev) => ({ ...prev, someoneDisconnected: false }));
        break;

      case 'su_session_expired':
        clearSessionId(SU_SESSION_KEY);
        setState((prev) => {
