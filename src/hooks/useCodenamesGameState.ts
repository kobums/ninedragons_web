...
interface CNState {
  ...
  gameOver: CNGameOverPayload | null;
  error: string | null;
  // 게임 중 사람 좌석이 끊겨 재접속 대기 중인지 (배너용)
  someoneDisconnected: boolean;
}

const initialState: CNState = {
  hasJoined: false,
  game: null,
  gameOver: null,
  error: null,
...
// 서버가 상태 변경마다 개인화 전체 스냅샷(cn_game_state)을 보내므로
...
export const useCodenamesGameState = (lastMessage: CNMessage | null) => {
  const [state, setState] = useState<CNState>(initialState);
  const [toasts, setToasts] = useState<CNToast[]>([]);
  ...
  useEffect(() => {
    ...
    switch (lastMessage.type) {
      case 'cn_player_joined': {
        const payload = lastMessage.payload as { sessionId?: string };
        if (payload?.sessionId) {
          saveSessionId(CN_SESSION_KEY, payload.sessionId);
        }
      ...
      case 'cn_game_state': {
        const game = lastMessage.payload as CNGameState;
        setState((prev) => ({
          ...prev,
          game,
          hasJoined: true,
      ...
      case 'cn_event': {
        const event = lastMessage.payload as CNEvent;
      ...
      case 'cn_game_over': {
        const gameOver = (lastMessage.payload ?? {}) as CNGameOverPayload;
        clearSessionId(CN_SESSION_KEY);
        setState((prev) => ({ ...prev, gameOver, someoneDisconnected: false }));
        break;
      }

      case 'cn_player_disconnected':
        setState((prev) => ({ ...prev, someoneDisconnected: true }));
        break;

      case 'cn_player_reconnected':
        // 정확한 접속 상태는 뒤따르는 cn_game_state 가 채운다
        setState((prev) => ({ ...prev, someoneDisconnected: false }));
        break;

      case 'cn_session_expired':
        clearSessionId(CN_SESSION_KEY);
        setState((prev) => {
