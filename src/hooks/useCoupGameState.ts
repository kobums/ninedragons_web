...
// 서버가 상태 변경마다 개인화 전체 스냅샷(cp_game_state)을 보내므로
...
export const useCoupGameState = (lastMessage: CPMessage | null) => {
  const [state, setState] = useState<CPState>(initialState);
  const [toasts, setToasts] = useState<CPToast[]>([]);
  ...
  useEffect(() => {
    ...
    switch (lastMessage.type) {
      case 'cp_player_joined': {
        const payload = lastMessage.payload as { sessionId?: string };
        if (payload?.sessionId) {
          saveSessionId(CP_SESSION_KEY, payload.sessionId);
        }
      ...
      case 'cp_game_state': {
        const game = lastMessage.payload as CPGameState;
        setState((prev) => ({
          ...prev,
          game,
          hasJoined: true,
      ...
      case 'cp_event': {
        const event = lastMessage.payload as CPEvent;
      ...
      case 'cp_game_over': {
        const gameOver = (lastMessage.payload ?? {}) as CPGameOverPayload;
        clearSessionId(CP_SESSION_KEY);
        setState((prev) => ({ ...prev, gameOver, someoneDisconnected: false }));
        break;
      }

      case 'cp_player_disconnected':
        setState((prev) => ({ ...prev, someoneDisconnected: true }));
        break;

      case 'cp_player_reconnected':
        // 정확한 접속 상태는 뒤따르는 cp_game_state 가 채운다
        setState((prev) => ({ ...prev, someoneDisconnected: false }));
        break;

      case 'cp_session_expired':
        clearSessionId(CP_SESSION_KEY);
        setState((prev) => {
