...
interface CWState {
  ...
  gameOver: CWGameOverPayload | null;
  error: string | null;
  // 게임 중 사람 좌석이 끊겨 재접속 대기 중인지 (배너용)
  someoneDisconnected: boolean;
}

const initialState: CWState = {
  hasJoined: false,
  game: null,
  gameOver: null,
  error: null,
...
// play·communicate 는 보드가 스냅샷(trick·players[].revealed)으로 직접 그린다.
const TOAST_SKIP: ReadonlySet<string> = new Set(['react']);

// 서버가 상태 변경마다 개인화 전체 스냅샷(cw_game_state)을 보내므로
// 이 훅은 스냅샷을 그대로 반영하고 토스트/에러만 따로 관리한다.
// (useKrakenGameState 결 — 다인 손패 은닉 스냅샷형)
export const useCrewGameState = (lastMessage: CWMessage | null) => {
  const [state, setState] = useState<CWState>(initialState);
  const [toasts, setToasts] = useState<CWToast[]>([]);
  ...
  useEffect(() => {
    ...
    switch (lastMessage.type) {
      case 'cw_player_joined': {
        const payload = lastMessage.payload as { sessionId?: string };
        if (payload?.sessionId) {
          saveSessionId(CW_SESSION_KEY, payload.sessionId);
        }
      ...
      case 'cw_game_state': {
        const game = lastMessage.payload as CWGameState;
        setState((prev) => ({
          ...prev,
          game,
          hasJoined: true,
      ...
      case 'cw_event': {
        const event = lastMessage.payload as CWEvent;
      ...
      case 'cw_game_over': {
        const gameOver = (lastMessage.payload ?? {}) as CWGameOverPayload;
        clearSessionId(CW_SESSION_KEY);
        setState((prev) => ({ ...prev, gameOver, someoneDisconnected: false }));
        break;
      }

      case 'cw_player_disconnected':
        setState((prev) => ({ ...prev, someoneDisconnected: true }));
        break;

      case 'cw_player_reconnected':
        // 정확한 접속 상태는 뒤따르는 cw_game_state 가 채운다
        setState((prev) => ({ ...prev, someoneDisconnected: false }));
        break;

      case 'cw_session_expired':
        clearSessionId(CW_SESSION_KEY);
        setState((prev) => {
