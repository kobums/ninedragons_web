...
interface IDState {
  ...
  gameOver: IDGameOverPayload | null;
  error: string | null;
...
// 서버가 상태 변경마다 개인화 전체 스냅샷(id_game_state)을 보내므로
...
export const useInsiderGameState = (lastMessage: IDMessage | null) => {
  const [state, setState] = useState<IDState>(initialState);
    ...
    switch (lastMessage.type) {
      case 'id_player_joined': {
        ...
        if (payload?.sessionId) {
          saveSessionId(ID_SESSION_KEY, payload.sessionId);
        }
      ...
      case 'id_game_state': {
        const game = lastMessage.payload as IDGameState;
        setState((prev) => ({
          ...prev,
          game,
          hasJoined: true,
      ...
      case 'id_event': {
        const event = lastMessage.payload as IDEvent;
      ...
      case 'id_game_over': {
        const gameOver = (lastMessage.payload ?? {}) as IDGameOverPayload;
        clearSessionId(ID_SESSION_KEY);
        setState((prev) => ({ ...prev, gameOver, someoneDisconnected: false }));
        break;
      }

      case 'id_player_disconnected':
        setState((prev) => ({ ...prev, someoneDisconnected: true }));
        break;

      case 'id_player_reconnected':
        // 정확한 접속 상태는 뒤따르는 id_game_state 가 채운다
