...
interface YTState {
  ...
  gameOver: YTGameOverPayload | null;
  error: string | null;
  // 게임 중 사람 좌석이 끊겨 재접속 대기 중인지 (배너용)
  someoneDisconnected: boolean;
}

const initialState: YTState = {
  hasJoined: false,
  game: null,
  gameOver: null,
  error: null,
...
// 서버가 상태 변경마다 전체 스냅샷(yt_game_state)을 보내므로
// 이 훅은 스냅샷을 그대로 반영하고 토스트/에러만 따로 관리한다.
// (useLoveLetterGameState 결 — 다인 스냅샷 훅)
export const useYachtGameState = (lastMessage: YTMessage | null) => {
  const [state, setState] = useState<YTState>(initialState);
  const [toasts, setToasts] = useState<YTToast[]>([]);
  ...
  useEffect(() => {
    ...
    switch (lastMessage.type) {
      case 'yt_player_joined': {
        const payload = lastMessage.payload as { sessionId?: string };
        if (payload?.sessionId) {
          saveSessionId(YT_SESSION_KEY, payload.sessionId);
        }
      ...
      case 'yt_game_state': {
        const game = lastMessage.payload as YTGameState;
        setState((prev) => ({
          ...prev,
          game,
          hasJoined: true,
      ...
      case 'yt_event': {
        const event = lastMessage.payload as YTEvent;
      ...
      case 'yt_game_over': {
        const gameOver = (lastMessage.payload ?? {}) as YTGameOverPayload;
        clearSessionId(YT_SESSION_KEY);
        setState((prev) => ({ ...prev, gameOver, someoneDisconnected: false }));
        break;
      }

      case 'yt_player_disconnected':
        setState((prev) => ({ ...prev, someoneDisconnected: true }));
        break;

      case 'yt_player_reconnected':
        // 정확한 접속 상태는 뒤따르는 yt_game_state 가 채운다
        setState((prev) => ({ ...prev, someoneDisconnected: false }));
        break;

      case 'yt_session_expired':
        clearSessionId(YT_SESSION_KEY);
        setState((prev) => {
