...
interface BZState {
  ...
  gameOver: BZGameOverPayload | null;
  error: string | null;
  // 게임 중 사람 좌석이 끊겨 재접속 대기 중인지 (배너용)
  someoneDisconnected: boolean;
}

const initialState: BZState = {
  hasJoined: false,
  game: null,
  gameOver: null,
  error: null,
...
// 심기·수확은 보드가 스냅샷(fields·lastAction)으로 직접 그리지만,
// 거래 성사·덱 소진처럼 놓치면 안 되는 사건은 토스트로도 남긴다.
const TOAST_SKIP: ReadonlySet<string> = new Set(['react']);

// 서버가 상태 변경마다 개인화 전체 스냅샷(bz_game_state)을 보내므로
// 이 훅은 스냅샷을 그대로 반영하고 토스트/에러만 따로 관리한다.
// (useStartupsGameState 결 — 다인 은닉 스냅샷형)
export const useBohnanzaGameState = (lastMessage: BZMessage | null) => {
  const [state, setState] = useState<BZState>(initialState);
  const [toasts, setToasts] = useState<BZToast[]>([]);
  ...
  useEffect(() => {
    ...
    switch (lastMessage.type) {
      case 'bz_player_joined': {
        const payload = lastMessage.payload as { sessionId?: string };
        if (payload?.sessionId) {
          saveSessionId(BZ_SESSION_KEY, payload.sessionId);
        }
      ...
      case 'bz_game_state': {
        const game = lastMessage.payload as BZGameState;
        setState((prev) => ({
          ...prev,
          game,
          hasJoined: true,
      ...
      case 'bz_event': {
        const event = lastMessage.payload as BZEvent;
      ...
      case 'bz_game_over': {
        const gameOver = (lastMessage.payload ?? {}) as BZGameOverPayload;
        clearSessionId(BZ_SESSION_KEY);
        setState((prev) => ({ ...prev, gameOver, someoneDisconnected: false }));
        break;
      }

      case 'bz_player_disconnected':
        setState((prev) => ({ ...prev, someoneDisconnected: true }));
        break;

      case 'bz_player_reconnected':
        // 정확한 접속 상태는 뒤따르는 bz_game_state 가 채운다
        setState((prev) => ({ ...prev, someoneDisconnected: false }));
        break;

      case 'bz_session_expired':
        clearSessionId(BZ_SESSION_KEY);
        setState((prev) => {
